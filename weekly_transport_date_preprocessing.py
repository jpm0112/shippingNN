...
df = df.drop(columns=['CIF TOTAL','US$ CIF','US$ CIF UNIT','US$ FOB UNIT','CANTIDAD','US$ FOB','US$ FLETE','US$ SEGURO','PAIS DE ADQUISICION','FOB TOTAL','FLETE TOTAL','SEGURO TOTAL',
...
daily_df = pd.DataFrame({'FECHA_DOC_TRANSPORTE': df['FECHA_DOC_TRANSPORTE'].dropna().sort_values().unique()})


din_diario = df.groupby('FECHA_DOC_TRANSPORTE')['NUMERO DE ACEPTACION'].nunique().reset_index()
din_diario.rename(columns={'NUMERO DE ACEPTACION': 'DIN_UNICOS_DIARIOS'}, inplace=True)

# Unir al DataFrame diario
daily_df = pd.merge(daily_df, din_diario, on='FECHA_DOC_TRANSPORTE', how='left')



# UNIQUE COUNT OF IMPORTER COMPANY AND PARTIDA ARACELARIA
rut_diario = df.groupby('FECHA_DOC_TRANSPORTE')['RUT PROBABLE IMPORTADOR'].nunique().reset_index()
rut_diario.rename(columns={'RUT PROBABLE IMPORTADOR': 'RUTS_UNICOS_DIARIOS'}, inplace=True)

partida_diario = df.groupby('FECHA_DOC_TRANSPORTE')['PARTIDA ARANCELARIA'].nunique().reset_index()
partida_diario.rename(columns={'PARTIDA ARANCELARIA': 'PARTIDAS_UNICAS_DIARIAS'}, inplace=True)

daily_df = daily_df.merge(rut_diario, on='FECHA_DOC_TRANSPORTE', how='left')
daily_df = daily_df.merge(partida_diario, on='FECHA_DOC_TRANSPORTE', how='left')
...
for col in cols:
    agg_df = df.groupby('FECHA_DOC_TRANSPORTE')[col].agg(agg_funcs).reset_index()
    agg_df.columns = ['FECHA_DOC_TRANSPORTE'] + [f'{func.upper()}_{col}' for func in agg_funcs]
    daily_df = pd.merge(daily_df, agg_df, how='left', on='FECHA_DOC_TRANSPORTE')



# CLAUSULA count
#todo I can group some categories in others
clausula_container_count = df.groupby(['FECHA_DOC_TRANSPORTE', 'CLAUSULA'])['CANTIDAD DE BULTO'].sum().reset_index()
pivot_clausula_count = clausula_container_count.pivot(index='FECHA_DOC_TRANSPORTE', columns='CLAUSULA', values='CANTIDAD DE BULTO').fillna(0).reset_index()
daily_df = pd.merge(daily_df, pivot_clausula_count, how='left', on='FECHA_DOC_TRANSPORTE')




# containers per company count
company_container_count = df.groupby(['FECHA_DOC_TRANSPORTE', 'COMPANIA DE TRANSPORTE'])['CANTIDAD DE BULTO'].sum().reset_index()
pivot_company_count = company_container_count.pivot(index='FECHA_DOC_TRANSPORTE', columns='COMPANIA DE TRANSPORTE', values='CANTIDAD DE BULTO').fillna(0).reset_index()
daily_df = pd.merge(daily_df, pivot_company_count, how='left', on='FECHA_DOC_TRANSPORTE')

# containers per container type count
container_type_container_count = df.groupby(['FECHA_DOC_TRANSPORTE', 'TIPO DE BULTO'])['CANTIDAD DE BULTO'].sum().reset_index()
pivot_container_type_count = container_type_container_count.pivot(index='FECHA_DOC_TRANSPORTE', columns='TIPO DE BULTO',
                                                    values='CANTIDAD DE BULTO').fillna(0).reset_index()
daily_df = pd.merge(daily_df, pivot_container_type_count, how='left', on='FECHA_DOC_TRANSPORTE')


# container per origin country
container_country_count = df.groupby(['FECHA_DOC_TRANSPORTE', 'PAIS DE ORIGEN'])['CANTIDAD DE BULTO'].sum().reset_index()
pivot_country_count = container_country_count.pivot(index='FECHA_DOC_TRANSPORTE', columns='PAIS DE ORIGEN',
                                                                  values='CANTIDAD DE BULTO').fillna(0).reset_index()
daily_df = pd.merge(daily_df, pivot_country_count, how='left', on='FECHA_DOC_TRANSPORTE')
...
for filename in os.listdir(folder):
    if filename.endswith(".csv"):
        ...
        if 'Date' in df_macro.columns:
            df_macro['FECHA_DOC_TRANSPORTE'] = pd.to_datetime(df_macro['Date'], errors='coerce')
            df_macro = df_macro.sort_values('FECHA_DOC_TRANSPORTE').drop(columns=['Date'])
            ...
            if 'Close' in df_macro.columns and 'Volume' in df_macro.columns:
                ...
                macro_filtered = df_macro[['FECHA_DOC_TRANSPORTE', 'Close', 'Volume', 'Close_pct_change']].rename(
                    columns={
                        'Close': f'{name}_price',
                ...
                daily_df = pd.merge(daily_df, macro_filtered, how='left', on='FECHA_DOC_TRANSPORTE')

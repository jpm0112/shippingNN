# bayesopt_transformer_ax.py
import warnings

from lightning import seed_everything
import numpy as np

warnings.filterwarnings("ignore")

from ax.service.ax_client import AxClient
from ax.service.utils.instantiation import ObjectiveProperties
from datetime import datetime
from pathlib import Path
import pandas as pd
import torch, csv

from functions import run_transformer, error_metrics

# ===== Data prep =====
deleted_sample = 0
test_size = 12

# 'FE', 'NAE', 'NAW', 'NE', 'SAE', 'SAW', 'SE'
target_col = 'SAW'
metric = "mape"  # objective to minimize "mape" or "r2"
minimize = True
country = 'chile'
df = pd.read_csv("weekly_chile_data.csv")


# ===== CSV =====
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
results_dir = Path("results");
results_dir.mkdir(parents=True, exist_ok=True)
csv_path = results_dir / f"transformer_trials_{country}_{target_col}_{timestamp}.csv"
csv_file = csv_path.open("w", newline="")
csv_writer = csv.DictWriter(csv_file, fieldnames=[
    "trial_index", "test_size",
    "window_size", "batch_size", "d_model", "n_head", "num_layers",
    "epoch_number", "lr",#"optimizer", #"weight_decay",
    "mae", "mape", "mse", "rmse", "r2", "runtime_s", "started_at","effective_epochs"
])
csv_writer.writeheader()

# ===== Ax setup =====
ax = AxClient()
ax.create_experiment(
    name="transformer_experiment",
    parameters=[
        {"name": "test_size", "type": "choice", "values": [12,24,48], "value_type": "int"},
        {"name": "window_size", "type": "range", "bounds": [12,96], "value_type": "int"},
        {"name": "d_model", "type": "choice", "values": [256, 512, 1024], "value_type": "int"},
        {"name": "n_head", "type": "choice", "values": [4,8,16,32], "value_type": "int"},
        {"name": "num_layers", "type": "range", "bounds": [1, 10], "value_type": "int"},
        {"name": "epoch_number", "type": "fixed", "value": 2000, "value_type": "int"},
        {"name": "batch_size", "type": "choice", "values": [16, 32, 64], "value_type": "int"},
        {"name": "lr", "type": "range", "bounds": [1e-6, 1e-2], "log_scale": True},
        # {"name": "optimizer", "type": "choice", "values": ['adam', "sgd", "adamw", 'Adagrad']},
        # {"name": "weight_decay", "type": "range", "bounds": [1e-6, 1e-3]},
    ],
    objectives={metric: ObjectiveProperties(minimize=minimize)},
)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
iterations = 100

for i in range(iterations):
    print("=== Trial %d ===" % (i + 1))
    print("________________________________")
    params, trial_index = ax.get_next_trial()
    started_at = datetime.now()

    tmp = df.copy().sort_values("FECHA")

    deleted_sample = int(params["test_size"])  #delete the test samples from the end
    if deleted_sample > 0:
        tmp = tmp.iloc[:-deleted_sample]

    d_model = int(params["d_model"])
    n_head = int(params["n_head"])
    if d_model % n_head != 0:
        ax.log_trial_failure(trial_index, metadata={"reason": "d_model % n_head != 0"})
        continue

    try:
        seed = 1048596
        seed_everything(seed)
        np.random.seed(seed)
        torch.manual_seed(seed)
        torch.cuda.manual_seed_all(seed)
        # === Train/eval ===
        y_true, y_pred, list = run_transformer(
            df=tmp,
            target_col=target_col,
            window_size=int(params["window_size"]),
            test_size=int(params["test_size"]),
            batch_size=int(params["batch_size"]),
            d_model=d_model,
            n_head=n_head,
            num_layers=int(params["num_layers"]),
            epoch_number=int(params["epoch_number"]),
            lr=float(params["lr"]),
            device=device,
            seed=seed,
            #optimizer=params["optimizer"],
            # weight_decay=float(params["weight_decay"]),
        )

        effective_epochs = list[4]
        print(f"Trial {trial_index} completed.")
        print("test_size ", params["test_size"])
        print("window_size ", params["window_size"])
        print("batch_size ", params["batch_size"])
        print("d_model ", d_model)
        print("n_head ", n_head)
        print("num_layers ", params["num_layers"])
        print("epoch_number ", params["epoch_number"])
        print("lr ", params["lr"])
        print("effective_epochs ", effective_epochs)

        mae, mape, mse, rmse, r2 = error_metrics(y_true, y_pred)
        runtime_s = (datetime.now() - started_at).total_seconds()
        if metric == "mape":
            ax.complete_trial(trial_index=trial_index, raw_data={metric: float(mape)})
        else:
            ax.complete_trial(trial_index=trial_index, raw_data={metric: float(r2)})

        csv_writer.writerow({
            "trial_index": trial_index,
            "test_size": int(params["test_size"]),
            "window_size": int(params["window_size"]),
            "batch_size": int(params["batch_size"]),
            "d_model": d_model,
            "n_head": n_head,
            "num_layers": int(params["num_layers"]),
            "epoch_number": int(params["epoch_number"]),
            "lr": float(params["lr"]),
            # "optimizer": params["optimizer"],
            # "weight_decay": float(params["weight_decay"]),
            "mae": mae, "mape": mape, "mse": mse, "rmse": rmse, "r2": r2,
            "runtime_s": runtime_s,
            "started_at": started_at.isoformat(timespec="seconds"),
            "effective_epochs": effective_epochs
        })
        csv_file.flush()

    except Exception as e:
        ax.log_trial_failure(trial_index, metadata={"exception": str(e)})

csv_file.close()
print(f"Wrote per-trial results to {csv_path.resolve()}")

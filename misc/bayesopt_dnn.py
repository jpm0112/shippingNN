...
csv_writer = csv.DictWriter(csv_file, fieldnames=[
    "trial_index",
    "window_size", "batch_size", "epoch_number", "lr", "hidden_size1", "hidden_size2", "hidden_size3", "dropout", "weight_decay",
...
client.configure_experiment(
    ...
    parameters=[
        ...
        RangeParameterConfig(name="weight_decay", bounds=(1e-8, 1e-2), parameter_type="float", scaling="log"),
        ChoiceParameterConfig(name="hidden_size1",
                              values=[32, 64, 96, 128, 160, 192, 256, 320, 384, 512],
                              parameter_type="int", is_ordered=True),
        ChoiceParameterConfig(name="hidden_size2",
                              values=[32, 64, 96, 128, 160, 192, 256, 320, 384, 512],
                              parameter_type="int", is_ordered=True),
        ChoiceParameterConfig(name="hidden_size3",
                        values=[32, 64, 96, 128, 160, 192, 256, 320, 384, 512],
                        parameter_type="int", is_ordered=True),
        RangeParameterConfig(name="dropout",      bounds=(0.0, 0.6), parameter_type="float"),
    ],
)
...
for _ in range(iterations):
    ...
    for trial_index, parameters in client.get_next_trials(max_trials=1).items():
        ...
        hidden_sizes = [parameters["hidden_size1"], parameters["hidden_size2"], parameters["hidden_size3"]]
        y_true, y_pred = run_dnn(
            df=df,
        ...
        csv_writer.writerow({
            ...
            "epoch_number": parameters["epoch_number"],
            "lr": parameters["lr"],
            "hidden_size1": parameters["hidden_size1"],
            "hidden_size2": parameters["hidden_size2"],
            "hidden_size3": parameters["hidden_size3"],
            "dropout": parameters["dropout"],
            "weight_decay": parameters["weight_decay"],
